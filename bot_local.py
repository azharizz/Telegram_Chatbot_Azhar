...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME = "Azhar Izzannada Elbachtiar",
            BATCH_ACADEMY = "Rhea Night",
            GITHUB_REPO_LINK = "https://github.com/azharizz/Telegram_Chatbot_Azhar"
        )
...
cols_cat = ['age', 'gender']
cols_obj = ['ad_id', 'campaign_id']

df[cols_cat] = df[cols_cat].astype('category')
df[cols_obj] = df[cols_obj].astype('object')
...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        df_plot = pd.pivot_table(df_campaign, values=['spent', 'approved_conversion'], index='age', aggfunc='sum')
        df_plot = df_plot.reindex(columns=['spent', 'approved_conversion'])
...
def echo_all(message):
    ...
    with open('template_text/default.txt', mode='r', encoding='utf-8') as f:
        ...
        default = temp.substitute(EMOJI = emoji.emojize(':worried_face:'))
